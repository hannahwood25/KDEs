...
FAME_CombinedBLKI_wTripIDs <- read_csv("~/Desktop/OMGPhD/PhD data/FAME_CombinedBLKI_wTripIDs.csv")
...
##### find out how many individuals tagged in each year

library(dplyr)

kitt_year_summary <- kitt %>%
  group_by(Year, Site) %>%
...
# Plot these trips
  ...
  # Create the plot using mapTrips and store it in an object
  ALLTRACKS <- mapTrips(trips = colony_trips, colony = colony_location)
  
  #### save a PDF of the unfiltered trips
  pdf(paste0(colony, "_unfiltered_trips.pdf"), width = 8, height = 8)
  print(ALLTRACKS)  # Print the stored plot
  dev.off()
  ...
  #### save a PDF of the complete trips
  # Create the plot using mapTrips and store it in an object
  COMPLETETRACKS <- mapTrips(trips = complete_colony_trips, colony = colony_location)
  
  #### save a PDF of the unfiltered trips
  pdf(paste0(colony, "_complete_trips.pdf"), width = 8, height = 8)
  print(COMPLETETRACKS)  # Print the stored plot
  dev.off()
 ...
 # print(long_colony_trips_summ)
  ...
  #### save a pdf of the KDEs
  KDEmap <- mapKDE(KDE = colony_KDE$UDPolygons, colony = colony_location)
  pdf(paste0(colony, "_kde_map.pdf"), width = 8, height = 8)
  print(KDEmap)  # Print the stored plot
  dev.off()
...
#all_trips_data = read.csv("/Users/hannahwood/Desktop/R 31st August/all_trips_data_summ_nofai.csv")
...
## save summary file
setwd("/Users/hannahwood/Desktop/R 31st August")
write.csv(all_trips_site_summary, "all_trips_site_summ_nofai.csv")
getwd()
