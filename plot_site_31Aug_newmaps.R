...
colonies_more_than_8 = subset(colonies_more_than_8, Site != "BAR")
colonies_more_than_8 = subset(colonies_more_than_8, Site != "FOW")
colonies_more_than_8 = subset(colonies_more_than_8, Site != "LAM")
colonies_more_than_8 = subset(colonies_more_than_8, Site != "SAB")
...
all_results_df_long$Behaviour <- all_results_df_long$behaviour

# Define your custom x-axis labels
custom_labels <- c("All Behaviour", "Sample", "Resting", "Foraging", "Commuting")
legend_labels <- custom_labels  # Use the same labels for the legend

# Create the ggplot plot with custom labels and legend title
ggplot(all_results_df_long, aes(x = Behaviour, y = log10(area_km), group = Behaviour, fill = Behaviour)) + 
  geom_boxplot(alpha = 0.4, width = 0.1) + 
  geom_dotplot(binaxis = 'y', stackdir = 'center', dotsize = 0.5, alpha = 0.4, position = position_dodge(1)) +
  geom_violin(alpha = 0.6, color = NA) +
  scale_fill_manual(values = c("skyblue", "#E7298A", "#1B9E77", "#D95F02", "#7570B3"),
                    labels = legend_labels) + 
  scale_x_discrete(labels = custom_labels) +  # Set custom x-axis labels
  labs(fill = "Behaviour") +  # Set legend title to "Behaviour"
  theme_bw()



###### Making the model of difference in site area for behaviours
