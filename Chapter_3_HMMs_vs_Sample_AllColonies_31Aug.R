  ...
  calculate_overlap = function(all_poly, behaviour_poly, this_behaviour = "unknown") {
    ...
    for (id in ids) {
      ...
      if ((nrow(this_all_poly) > 0) & (nrow(this_behaviour_poly) > 0)) {
        ...
        intersection = st_intersection(this_all_poly, this_behaviour_poly)
        
        # Work out area of all, behaviour and intersection
        behaviour_area = st_area(this_behaviour_poly)
        all_area = st_area(this_all_poly)
        ...
        if (nrow(intersection) == 0) {
          ...
          area_intersect = st_area(intersection)
        }
